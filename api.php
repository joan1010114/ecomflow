...
// V4.9 (Q7): 批次更新
if ($path === 'batch_update' && $method === 'POST') {
    $body = body();
    $tb = $body['table'] ?? '';
    $ids = $body['ids'] ?? [];
    $changes = $body['changes'] ?? [];
    if (!$tb || !in_array($tb, $validTables)) err('Invalid table');
    if (empty($ids) || !is_array($ids)) err('No ids');
    if (empty($changes) || !is_array($changes)) err('No changes');
    enforcePermission($pdo, $currentUser, $tb, 'PUT');

    // 處理 JSON / credential 加密
    $changes = prepareRow($tb, $changes);
    $changes = processCredentials($tb, $changes, true);
    $changes = filterValidColumns($pdo, $tb, $changes);
    if (empty($changes)) err('No valid fields');
    ...
    foreach ($ids as $id) {
        ...
        try {
            ...
            $vals[] = $id;
            $stmt = $pdo->prepare("UPDATE `$tb` SET $set WHERE id = ?");
            $stmt->execute($vals);
            if (shouldLogTable($tb)) logActivity($pdo, $currentUser, 'update', $tb, $id, ['fields' => array_keys($changes), 'batch' => true]);
            $success++;
        } catch (Exception $e) {
            $failed++;
            if (count($errors) < 5) $errors[] = $e->getMessage();
...
// V4.9 (Q7): 批次刪除
if ($path === 'batch_delete' && $method === 'POST') {
    $body = body();
    $tb = $body['table'] ?? '';
    $ids = $body['ids'] ?? [];
    if (!$tb || !in_array($tb, $validTables)) err('Invalid table');
    if (empty($ids) || !is_array($ids)) err('No ids');
    enforcePermission($pdo, $currentUser, $tb, 'DELETE');
    ...
    foreach ($ids as $id) {
        ...
        try {
            ...
            $stmt->execute([$id]);
            if (shouldLogTable($tb)) logActivity($pdo, $currentUser, 'delete', $tb, $id, ['batch' => true]);
            $success++;
        } catch (Exception $e) {
            $failed++;
        }
